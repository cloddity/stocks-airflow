...
def run_ctas(table, select_sql, primary_key=None):
    ...
    try:
        ...
        sql = f"""SELECT userId, sessionId, ts, COUNT(*) FROM {table} GROUP BY userId, sessionId, ts HAVING COUNT(*) > 1;"""
        cur.execute(sql)
        result = cur.fetchone()
        print(type(result))
        if result is not None:
            print("!!!!!!!!!!!!!!")
            raise Exception(f"Duplicate check failed: {result}")
